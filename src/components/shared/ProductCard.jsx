...
export default function ProductCard({ product, onAddToCart, user, listView = false }) {
  ...
  return (
    <div className="group bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 active:scale-[0.97] flex flex-col">
      {/* Image - Premium centered white bg */}
      <Link to={createPageUrl("ProductDetail") + `?id=${product.id}`} className="relative block">
        <div className="relative aspect-square bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 overflow-hidden flex items-center justify-center border-b border-slate-100 dark:border-slate-800">
          {product.image_url ? (
            <img
              src={product.image_url}
              alt={product.name}
              className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-500 p-5"
              loading="lazy"
            />
          ) : (
            <div className="w-full h-full flex items-center justify-center">
          ...
          {product.original_price && product.original_price > product.price && (
            <div className="absolute top-3 right-3 bg-gradient-to-br from-red-500 to-red-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md">
              -{Math.round(((product.original_price - product.price) / product.original_price) * 100)}%
            </div>
          )}
          ...
          {isOutOfStock && (
            <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center">
              <span className="text-white text-xs font-bold bg-slate-900/80 px-3 py-1.5 rounded-full">Out of stock</span>
            </div>
          )}

          <div className="absolute top-3 right-3" onClick={(e) => e.preventDefault()}>
            <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-md rounded-full p-2 shadow-md hover:shadow-lg transition-shadow">
              <WishlistButton product={product} />
            </div>
          </div>
        </div>
      ...
      {/* Body - enhanced typography and spacing */}
      <div className="p-4 flex flex-col flex-1 gap-2.5">
        {/* Shop name - subtle */}
        <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide truncate">{product.shop_name}</p>

        {/* Product name - prominent */}
        <Link to={createPageUrl("ProductDetail") + `?id=${product.id}`} className="flex-1">
          <h3 className="font-semibold text-sm text-slate-900 dark:text-slate-50 line-clamp-2 leading-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
            {product.name}
          </h3>
        </Link>

        {/* Price - bold and prominent */}
        <div className="flex items-baseline gap-2.5 flex-wrap py-1">
          <span className="text-lg font-bold text-slate-900 dark:text-slate-100">K{product.price?.toLocaleString()}</span>
          {product.original_price && product.original_price > product.price && (
            <span className="text-xs text-slate-400 dark:text-slate-500 line-through font-medium">K{product.original_price?.toLocaleString()}</span>
          )}
        </div>
        ...
        {canAddToCart && (
          <div className="mt-auto pt-1">
            <Button
              size="sm"
              disabled={isOutOfStock}
              className="w-full h-9 text-xs font-semibold bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white active:scale-95 transition-all shadow-sm hover:shadow-md rounded-lg gap-1.5"
              onClick={(e) => { e.preventDefault(); onAddToCart?.(product); }}
            >
