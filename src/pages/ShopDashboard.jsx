  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex">
        ...
        <main className="flex-1 pt-20 lg:pt-8 p-4 lg:p-8 overflow-auto min-w-0 text-slate-900 dark:text-slate-100" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 4.5rem)" }}>
            ...
            <Dialog open={productDialog} onOpenChange={setProductDialog}>
              <DialogContent className="max-w-lg">
                ...
                <div className="space-y-4 max-h-[60vh] overflow-auto pr-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div><Label>Category</Label><div className="mt-1"><MobileSelect value={productForm.category} onValueChange={v => setProductForm({...productForm, category: v})} placeholder="Category" options={CATEGORIES} /></div></div>
                    <div><Label>Condition</Label><div className="mt-1"><MobileSelect value={productForm.condition} onValueChange={v => setProductForm({...productForm, condition: v})} placeholder="Condition" options={[{value:"new",label:"New"},{value:"used",label:"Used"},{value:"refurbished",label:"Refurbished"}]} /></div></div>
                  </div>
                ...
                <Card key={t.id} className="border-slate-100 dark:border-slate-700 dark:bg-slate-900">
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center overflow-hidden flex-shrink-0">
                        {t.photo_url ? <img src={t.photo_url} alt="" className="w-full h-full object-cover" /> : <span className="text-sm font-bold text-blue-600 dark:text-blue-400">{t.name?.charAt(0)?.toUpperCase() || "T"}</span>}
                      </div>
                      <div>
