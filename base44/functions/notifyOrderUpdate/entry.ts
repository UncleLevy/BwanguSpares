...
Deno.serve(async (req) => {
  try {
    ...
    let { data, old_data } = payload;

    // If payload was too large, fetch the order data directly
    if (payload_too_large || !data) {
      try {
        data = await base44.asServiceRole.entities.Order.get(event.entity_id);
      } catch (err) {
        console.warn(`Could not fetch order ${event.entity_id}:`, err.message);
        return Response.json({ success: true, skipped: 'could not fetch order data' });
      }
    }

    if (!data || !data.id) {
      console.warn('No order data available, skipping.');
      return Response.json({ success: true, skipped: 'no data' });
    }
    ...
    if (event.type === 'create') {
      ...
      // ── 2. Notify shop owner + send receipt ───────────────────────────────
      let shop = null;
      try {
        if (data.shop_id) {
      ...
      if (shop) {
        try {
          await base44.asServiceRole.entities.Notification.create({
            ...
            message: `You have a new order from ${data.buyer_name || data.buyer_email} worth K${data.total_amount}`,
            related_id: data.id,
            action_url: 'ShopDashboard?view=orders',
          });
        } catch (err) {
          console.warn('Shop notification failed:', err.message);
        }
        ...
        try {
          ...
          const subtotal = receiptItems.reduce((s, i) => s + (i.price || 0) * (i.quantity || 1), 0);
      ...
      if (!statusChanged) {
        return Response.json({ success: true, skipped: 'no status change' });
      }

      // ── 3. Notify buyer of status change ──────────────────────────────────
      try {
        await base44.asServiceRole.entities.Notification.create({
        ...
        console.warn('Buyer status notification failed:', err.message);
      }

      // ── 4. On delivery: review reminder + loyalty points ──────────────────
      if (data.status === 'delivered') {
        try {
          ...
          console.warn('Review reminder notification failed:', err.message);
        }
        ...
        if (amountSpent > 0) {
          try {
            ...
            console.warn('Loyalty points processing failed:', err.message);
          }
        }
      }
    }
    ...
    console.error('notifyOrderUpdate fatal error:', error.message);
